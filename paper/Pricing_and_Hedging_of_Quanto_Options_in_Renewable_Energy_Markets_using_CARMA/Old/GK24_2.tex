...
The paper is theoretical. We do not report an empirical calibration exercise
here, but the state-space structure is chosen precisely because it supports
calibration, filtering, and simulation routines already established in the
CARMA literature. Relative to the literature on energy quantos, the novelty is
therefore not a new numerical scheme in isolation, but a coherent mathematical
framework that links CARMA dynamics, measure selection, transform pricing, and
incomplete-market hedging in a single model class.
...
in \Cref{sec:hedging-strategies}. \Cref{sec:limitations} highlights limitations
and extensions, and \Cref{sec:conclusion} concludes.
...
\((Z_t)_{t \ge 0}\) solves
\begin{equation}\label{eq:state_general}
...
\begin{equation}\label{eq:variation_constants}
...
\begin{equation}\label{eq:stationary_solution}
...
\begin{equation}\label{eq:single_factor_cf}
...
\begin{equation}\label{eq:coupled_model}
\begin{aligned}
  \dd Z_t^{Y,\mathbb{M}}
...
\begin{equation}\label{eq:outputs_coupled}
...
\begin{equation}\label{eq:sylvester_cross_cov}
...
\end{equation}
For \(h \ge 0\),
\begin{equation}\label{eq:lagged_cross_cov}
...
\begin{align*}
  \overline{X}_t(T)
    &\coloneqq \Lambda_S(T)
      + c_X^\top \ee^{A_X^\Q (T-t)} Z_t^{X,\Q},\\
  \overline{Y}_t(T)
    &\coloneqq \Lambda_Y(T)
      + c_Y^\top \ee^{A_Y^\Q (T-t)} Z_t^{Y,\Q},
\end{align*}
...
\begin{proof}
Because \(S_T = \ee^{X_T}\), the forward price is
\(\E^\Q[S_T\mid\F_t] = \E^\Q[\ee^{X_T}\mid\F_t]\).
...
\begin{equation}\label{eq:quanto_payoff_general}
  g(s,y) = (s-K_S)^+ h(y),
\end{equation}
...
\[
H = \sum_{k=1}^n w_k g(S_{t_k},Y_{t_k}),
\]
...
transform space. Let \(x=\log s\) and define the damped payoff
\[
g_\alpha(x,y) \coloneqq \ee^{-\alpha_1 x-\alpha_2 y} g(\ee^x,y),
\]
for a damping vector \(\alpha=(\alpha_1,\alpha_2)\in\R^2\). Denote by
\[
\widehat{g_\alpha}(u,v)
= \int_{\R^2} \ee^{-\ii(ux+vy)} g_\alpha(x,y)\,\dd x\,\dd y
\]
...
\begin{enumerate}[label=(\roman*)]
  \item \(g_\alpha \in L^1(\R^2)\), so that \(\widehat{g_\alpha}\) exists and
    is bounded;
  \item the conditional moment condition
    \[
    \E^\Q\!\left[
      \ee^{\alpha_1 X_T + \alpha_2 Y_T}
      \left|g(\ee^{X_T},Y_T)\right|
      \,\middle|\, \F_t
    \right] < \infty
    \]
  ...
  \item the product \((u,v) \mapsto
    |\widehat{g_\alpha}(u,v)|\cdot|\varphi_t(u-\ii\alpha_1,v-\ii\alpha_2;T)|\)
...
\begin{equation}\label{eq:fourier_pricing}
  V_t
...
\begin{proof}
Since \(g_\alpha \in L^1(\R^2)\), Fourier inversion gives
\[
g(\ee^x,y)
= \frac{\ee^{\alpha_1 x+\alpha_2 y}}{(2\pi)^2}
   \int_{\R^2}
      \ee^{\ii(ux+vy)} \widehat{g_\alpha}(u,v)\,\dd u\,\dd v.
\]
...
\((S_T-K_S)^+(K_Y-Y_T)^+\), the damped transform factorizes into a call part and
...
\begin{equation}\label{eq:weak_coupling_model}
\begin{aligned}
  \dd Z_u^{Y}
...
\[
V_t^\lambda
= \frac{B_t^d}{B_T^d}\E^\Q\!\left[(S_T^\lambda-K_S)^+h(Y_T)\mid\F_t\right]
\]
...
\[
\frac{\partial}{\partial\lambda}(S_T^\lambda - K_S)^+ h(Y_T)
= S_T^\lambda \1_{\{S_T^\lambda>K_S\}} \Xi_{t,T} h(Y_T).
\]
...
absolute value by
\(\sup_{\lambda\in[-1,1]}S_T^\lambda\cdot|\Xi_{t,T}|\cdot|h(Y_T)|\), and the
...
assumption \(\E^\Q[|h'(Y_T)|\mid\F_t]<\infty\).
Hence
\begin{align*}
  \left.\frac{\partial V_t^\lambda}{\partial \lambda}\right|_{\lambda=0}
  &= \frac{B_t^d}{B_T^d}
     \E^\Q\!\left[
       S_T^0 \1_{\{S_T^0>K_S\}} \Xi_{t,T} h(Y_T)
       \mid \F_t
     \right].
\end{align*}
...
\begin{align*}
  ...
  &= \frac{B_t^d}{B_T^d}
     \E^\Q\!\left[S_T^0 \1_{\{S_T^0>K_S\}}\mid\F_t\right]
     \E^\Q\!\left[\Xi_{t,T} h(Y_T)\mid\F_t\right].
\end{align*}

Conditionally on \(\F_t\), the pair \((\Xi_{t,T},Y_T)\) is jointly Gaussian.
Indeed, \(\Xi_{t,T}\) is a Gaussian stochastic integral and \(Y_T =
\overline{Y}_t(T) + c_Y^\top\int_t^T \ee^{A_Y(T-s)} B_Y\,\dd W_s\) is a
Gaussian functional of the same Brownian motion. Moreover,
...
\(\E^\Q[|h'(Y_T)|\mid\F_t]<\infty\). Substituting into the previous line
...
payoff \((K_Y-Y_T)^+\) receives a negative first-order correction. This matches
...
\begin{equation}\label{eq:hedge_ratio}
  \xi_t
...
\(\xi_t \in \R^m\) is
\begin{equation}\label{eq:multi_hedge}
  \xi_t
...
\begin{equation}\label{eq:forward_qv}
...
\begin{equation}\label{eq:hedge_ratio_carma}
  \xi_t
...
We have developed a coherent mathematical framework for pricing and hedging
quanto options in CARMA-based models. The central idea is to combine a tradable
energy factor and a secondary hybrid-risk factor in a coupled CARMA state-space
system. This specification is rich enough to capture non-trivial dependence and
Lévy-driven non-Gaussian behavior, but structured enough to retain explicit
transforms, forward-price formulas, and covariance equations.

The analysis leads to three main conclusions. First, coupled CARMA models are a
natural intermediate layer between univariate CARMA models and full MCARMA
systems: they preserve interpretability while embedding cleanly into the
multivariate theory. Second, pricing in hybrid markets must distinguish clearly
between the tradable and the non-tradable parts of the claim. The correct
pricing object is therefore the conditional transform under a chosen pricing
measure, not an indiscriminate application of complete-market risk-neutral
valuation. Third, hedging is fundamentally an incomplete-market problem. The
variance-optimal decomposition makes explicit which component of quanto risk is
projectable onto traded instruments and which component remains orthogonal and
therefore unhedgeable.

From a modeling perspective, the main value of the CARMA framework is that it
keeps these pricing and hedging questions within a tractable linear state-space
setting. This makes the model suitable not only for theoretical analysis but
also for calibration, transform pricing, and hedging backtests, which are the
natural next steps in turning the present theoretical framework into a full
empirical study.

Concretely, a calibration program would proceed as follows. First, estimate the
marginal CARMA orders and parameters of the energy and secondary factor
separately using standard methods (likelihood, quasi-likelihood, or moment
matching). Second, identify the coupling matrix \(\Gamma\) from the empirical
lagged cross-covariance \eqref{eq:lagged_cross_cov} using least squares or
generalized method of moments. Third, calibrate the pricing measure via an
Esscher tilt of the secondary factor to match hybrid OTC quotes or actuarial
benchmarks. Finally, implement the variance-optimal hedge dynamically by
computing the forward quadratic covariation
\eqref{eq:hedge_ratio} from state estimates delivered by a Kalman or particle
filter applied to the stacked state vector. The complete multivariate CARMA
formulation (\Cref{prop:mcarma_embedding}) provides a natural setting for
multivariate filtering and for extending the hedging universe to include
additional tradable instruments.
